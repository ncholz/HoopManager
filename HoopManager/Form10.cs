...
namespace HoopManager
{
    public partial class Form10 : Form
    {
        ...
        int idSel = 0; // ID del jugador seleccionado para editar/borrar
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connection))
                {
                    ...
                    string sql = @"SELECT j.id, j.nombre, j.posicion, j.altura_cm, j.dorsal, 
                           e.nombre AS Equipo, j.id_equipo 
                           FROM jugadores j 
        ...
        {
            int idJugador = Convert.ToInt32(dgvJugadores.CurrentRow.Cells["id"].Value);
            string nombre = dgvJugadores.CurrentRow.Cells["nombre"].Value.ToString();

            if (PuedeBorrarse(idJugador))
            {
                // ESCENARIO: JUGADOR SIN REGISTROS
                ...
                if (res == DialogResult.Yes)
                    EjecutarBorradoReal(idJugador);
                else if (res == DialogResult.No)
                    EjecutarBorradoLogico(idJugador);
            }
            else
            {
                // ESCENARIO: JUGADOR CON REGISTROS
        ...
        private void dgvJugadores_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                // Cargamos los NumericUpDown desde la tabla
        ...
        //arreglar lo de q al tener un jugador inactivo, se active con el boton de borrar PERO cambiando el nombre al botón al tener un jugador inactivo seleccionado en la celda
    }
}